...
namespace MyTools.Services
{
    ...
    public static class ScheduleExcelExporter
    {
        ...
        private const string HolidayBg = "BFDBFE";
        private const string CellBorder = "E2E8F0";
        private const string White = "FFFFFF";
        private const string EmptyHolidayBg = "BFDBFE";
        ...
        // 节假日列"白"格子的蓝底
        private const string HolidayDayBg = "BFDBFE";
        ...
        private static async Task WriteWorksheetAsync(XmlWriter w, ScheduleVersion sched, StyleTable styles)
        {
            ...
            foreach (var emp in sched.Employees)
            {
                ...
                for (int d = 0; d < days; d++)
                {
                    ...
                    if (!string.IsNullOrEmpty(code) && code != ShiftCodes.Day && ShiftColors.ContainsKey(code))
                    {
                ...
                // 统计
                var weekendRest = ScheduleStatistics.ComputeWeekendRestDays(sched, emp);

...
{
    public sealed class PostgreSqlExportProvider : ISqlExportProvider
    {
        public SqlProviderKind ProviderKind => SqlProviderKind.PostgreSql;

        public async Task TestConnectionAsync(SqlServerConnectionOptions options, CancellationToken cancellationToken)
        {
            ValidateConnectionOptions(options);
            AppLogService.Information("Testing PostgreSQL connection to {ServerAddress}:{Port}", options.ServerAddress, NormalizePort(options.Port, 5432));
            using (var connection = new NpgsqlConnection(BuildMasterConnectionString(options)))
            {
        ...
        public async Task<List<DatabaseItem>> GetDatabasesAsync(SqlServerConnectionOptions options, CancellationToken cancellationToken)
        {
            ...
            AppLogService.Information("Loaded {DatabaseCount} PostgreSQL databases from {ServerAddress}", result.Count, options.ServerAddress);
            return result;
        }
...
ORDER BY 1,2;";
            ...
            using (var connection = new NpgsqlConnection(BuildDatabaseConnectionString(options, databaseName)))
            using (var command = new NpgsqlCommand(sql, connection))
            {
                await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
                using (var reader = await command.ExecuteReaderAsync(CommandBehavior.CloseConnection, cancellationToken).ConfigureAwait(false))
            ...
            AppLogService.Information("Loaded {TableCount} PostgreSQL tables from {DatabaseName}", result.Count, databaseName);
            return result;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            AppLogService.Information(
                "PostgreSQL export completed for {DatabaseName}.{SchemaName}.{TableName} with {RowCount} rows",
                databaseName,
                table.SchemaName,
                table.TableName,
                rowCount);
        ...
        private static string BuildMasterConnectionString(SqlServerConnectionOptions options)
        {
            ...
            {
                Host = options.ServerAddress?.Trim(),
                Port = NormalizePort(options.Port, 5432),
                Database = "postgres",
        ...
        {
            var builder = new NpgsqlConnectionStringBuilder(BuildMasterConnectionString(options))
            {

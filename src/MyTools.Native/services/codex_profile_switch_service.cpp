...
std::string BuildActiveJson(const std::wstring& display_name) {
    SYSTEMTIME utc{};
    GetSystemTime(&utc);
    ...
    json << "{\n"
         << "  \"ActiveDisplayName\": \"" << JsonEscape(WideToUtf8(display_name)) << "\",\n"
         << "  \"SwitchedAtUtc\": \"" << JsonEscape(WideToUtf8(timestamp)) << "\"\n"
         << "}\n";
    return json.str();
}
...
bool CodexProfileSwitchService::ApplyProfileByDisplayName(const CodexProfileSwitchRequest& request,
                                                          CodexProfileSwitchResult* result) const {
    if (result == nullptr) {
        return false;
    }
    *result = CodexProfileSwitchResult{};
    ...
    if (protected_config.empty() || protected_auth.empty()) {
        ...
        result->error_message = L"Target Codex profile does not contain embedded config/auth content.";
        return false;
    }
    ...
    CodexProfileBackupService backup_service;
    CodexCurrentFolderBackupResult backup_result;
    if (!backup_service.CreateCurrentFolderBackup(request.codex_home,
                                                  request.backup_directory,
                                                  request.previous_active_display_name,
                                                  &backup_result)) {
        ClearString(&config_toml);
        ClearString(&auth_json);
    ...
    result->backup_skipped_no_files = backup_result.skipped_no_files;
    ...
    if (!FileSystem::WriteUtf8FileAtomic(config_path, config_toml, &result->error_message) ||
        !FileSystem::WriteUtf8FileAtomic(auth_path, auth_json, &result->error_message)) {
        ClearString(&config_toml);
        ClearString(&auth_json);
        return false;
    }
    ...
    std::string active_json = BuildActiveJson(request.target_display_name);
    if (!FileSystem::WriteUtf8FileAtomic(request.active_json_path, active_json, &result->error_message)) {
